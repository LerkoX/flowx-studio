...
- `execution continue` 用于已结束（success/failed/cancelled）的执行实例：不带 `--file` 时增量重跑（已终结节点跳过）；带 `--file` 时先用新 YAML 比对更新图（可追加节点，Version/Name 不可变、已执行节点不可删改），再继续运行。续跑沿用同一执行 ID，日志与节点记录追加，可通过 `execution logs/nodes/get` 查询。该操作只影响此执行实例，不会修改流水线定义本身（需要时用 `pipeline update`）。续跑基于执行结束时导出的运行时快照（DB `runtime_yaml`）恢复，server 重启后依然可用；仅快照功能上线前的旧执行无法续跑。
