...
// saveRuntimeSnapshot 在执行完成的 WorkflowFinish 事件内同步保存快照 YAML
...
// WorkflowPaused 事件经事件桥落库为 paused（见 persistRuntimeEvent）。
// 暂停即导出运行时快照（此处一次 + WorkflowPaused 事件一次），server 重启后
...
func (s *WorkflowService) PauseExecution(execID int64) error {
	...
	// WorkflowPaused 事件会再次导出干净状态的快照覆盖之
	if snapshot, err := s.runtime.ExportExecutionConfig(execID); err == nil && snapshot != "" {
		s.saveRuntimeSnapshot(execID, snapshot)
	}
	// 立即落库并广播：暂停在层边界才生效，若等引擎 WorkflowPaused 事件再更新，
...
func (s *WorkflowService) ResumeExecution(execID int64) error {
	...
	// 立即落库并广播：内存内 Resume 时引擎 WorkflowResumed 事件为幂等重复；
...
// Base/JobID 为新增可选字段（节点级中断与 op-replay 重放用），旧节点包不上报。
...
func (s *WorkflowService) handlePreviewMarker(execID int64, nodeID string, payload previewMarkerPayload) {
	...
	src.updatedAt = now
	if payload.Base != "" {
		src.base = strings.TrimRight(payload.Base, "/")
	}
	if payload.JobID != "" {
		src.jobID = payload.JobID
	}
...
func (s *WorkflowService) nodeConnectionOf(execID int64, nodeID string) (base, tok, jobID string) {
	...
	if src := s.previewSrcs[key]; src != nil {
		base, tok, jobID = src.base, src.token, src.jobID
	}
	s.previewMu.Unlock()
	if base != "" {
		return
	...
	exec, err := s.GetExecution(execID)
	if err != nil || exec == nil || exec.MetadataJSON == nil {
		return
	}
	var snap map[string]interface{}
	if json.Unmarshal([]byte(*exec.MetadataJSON), &snap) != nil {
		return
	}
	params, _ := snap["params"].(map[string]interface{})
	if v, ok := params["service_url"].(string); ok {
		base = strings.TrimRight(v, "/")
	}
	if tok == "" {
		if v, ok := params["service_token"].(string); ok {
			tok = v
		}
	}
	return
