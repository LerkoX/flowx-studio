...
节点 stdout/stderr 的输出行默认记为 `info` 级日志。若某行以以下前缀开头，运行时剥离前缀后按对应级别入库，前端日志面板按级别着色/提示：

| 前缀 | 级别 |
| --- | --- |
| `[flowx:error]` | error（节点异常、失败原因必须用它，否则前端无失败提示） |
| `[flowx:warn]` | warn |
| `[flowx:debug]` | debug |

注意：标记按「行」识别，必须位于行首；多行错误（如堆栈）只有首行能提级，其余行用普通输出即可。
