...
import { X, GitBranch, Container, Plus, Loader2 } from 'lucide-react'

interface NodeImportModalProps {
  isOpen: boolean
  onClose: () => void
  onAdd: (data: { type: 'git' | 'image'; url: string }) => void
  isAdding: boolean
}

export default function NodeImportModal({ isOpen, onClose, onAdd, isAdding }: NodeImportModalProps) {
  const [activeTab, setActiveTab] = useState<'git' | 'image'>('git')
              ...
              <div className="flex border-b border-white/10">
                <button
                  ...
                  className={`flex-1 py-3 text-sm font-medium transition-all relative
                    ${activeTab === 'git' ? 'text-white' : 'text-white/40 hover:text-white/60'}`}
                >
                ...
                <button
                  ...
                  className={`flex-1 py-3 text-sm font-medium transition-all relative
                    ${activeTab === 'image' ? 'text-white' : 'text-white/40 hover:text-white/60'}`}
                >
                  <span className="flex items-center justify-center gap-2">
                    <Container size={14} />
                    Docker 镜像
              ...
              <form onSubmit={handleSubmit} className="p-6 space-y-4">
                <div>
                  <label className="block text-white/60 text-xs mb-2">
                    {activeTab === 'git' ? 'Git 仓库地址' : 'Docker 镜像地址'}
                  </label>
                  <input
                    type="text"
                    value={url}
                  ...
                  <p className="text-white/30 text-[11px] mt-2">
                    {activeTab === 'git'
                      ? '支持公开 Git 仓库，系统将自动克隆并解析 flowx.json'
                      : '系统将拉取镜像并解析镜像中的节点信息'}
                  </p>
                </div>
